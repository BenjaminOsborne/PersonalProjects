...
open HelperTypes;
    ...
    let SolveProblem15 nIgnore = 
        let fnFactorial nNum = {1L..nNum} |> Seq.fold (fun x acc -> x * acc) 1L
        let fnMultipleRange nStart nStep nEnd = {nStart..nStep..nEnd} |> Seq.fold (fun x acc -> x * acc) 1L
    ...
    let SolveProblem23 nIgnore = 
        let fnGetSumDivisors nNum =
            ...
            {1..int(System.Math.Sqrt(float(nNum)))} |> Seq.filter (fun x -> nNum % x = 0) |> Seq.map (fun x -> fnGetSum x nNum) |> Seq.sum
    ...
    let SolveProblem24 nIgnore = 
        let arrArrays = HelperFunctions.GetAllPandigitals()
        let arrResult = arrArrays.[1000*1000-1];
    ...
    let SolveProblem26 nIgnore = 
        ...
        let listCycles = HelperFunctions.BuildPrimes(1000) |> Seq.map (fun nDenom -> (nDenom,fnFindCycleLength nDenom)) |> Seq.sortBy (fun (a,b) -> b) |> Seq.toList |> List.rev
    ...
    let SolveProblem35 nIgnore = 
        ...
        let listPrimes = HelperFunctions.BuildPrimes(1000*1000)
    ...
    let SolveProblem39 nIgnore = 
        ...
        let fnGetHypot a b = int(System.Math.Sqrt(float(a*a + b*b)))
    ...
    let SolveProblem41 nIgnore = 
        ...
        let nMaxSize = 1000*1000;
        let nInitial = 100

        let fnGetPrimesTo nNum = 
            let nBlocks = nNum /nMaxSize + 1
            fnPrint "Size Array"
            let arrNums = {0..nBlocks-1} |> Seq.map(fun block -> Array.init(nMaxSize+1)(fun x -> true) ) |> Seq.toArray
            fnPrint "End Array"
            
            let fnGetVal nNum = 
                let nBlock = nNum / nMaxSize;
                let nRemainder = nNum - (nMaxSize*nBlock)
            ...
            let fnCancelMultiples nVal =
                if fnGetVal nVal then
                    fnPrint ("Processing " + nVal.ToString() + " of " + nMax.ToString())
            ...
            let fnGetBatches nLow nHigh = 
                let nNum = nHigh-nLow;
                let nBatches = nNum/nMaxSize;
                let nRemainder = nNum - nBatches*nMaxSize
                seq { yield! {0..nBatches-1} |> Seq.map(fun x -> {nLow+nMaxSize*x..(nLow+nMaxSize*(x+1))-1} |> Seq.toList)     
                      yield {nLow+nBatches*nMaxSize..nHigh} |> Seq.toList }

            fnPrint "Initial Primes Start"
            fnParrallelProcess(HelperFunctions.BuildPrimes(nInitial) |> Seq.map (fun x -> [x])) //Process initial block of primes 1 per thread
            
            fnPrint "Batch Start"
            let seqBatches = fnGetBatches (nInitial+1) nMax
            fnParrallelProcess seqBatches
            
            fnPrint "Process Primes Start"
        ...
        let listPrimes = fnGetPrimesTo (987654321) |> Seq.filter fnIsPandigital |> Seq.toList
        
        fnPrint "All Done"

        let nTop = listPrimes |> Seq.last
        nTop; //7652413

    let SolveProblem42 nIgnore = 
        let arrPandigitals = HelperFunctions.GetAllPandigitals();
        let arrPrimes = HelperFunctions.BuildPrimes(17) |> Seq.toArray
